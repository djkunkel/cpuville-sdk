...
current_location:	defs 2		;word variable in RAM
line_count:			defs 1		;byte variable in RAM
byte_count:			defs 1		;byte variable in RAM
value_pointer:		defs 2		;word variable in RAM
current_value:		defs 2		;word variable in RAM
buffer:				defs 247,0		;buffer in RAM 
...
portwr_entry_msg:	defm	"Enter port number (hex, 0 to F): ",0
portwr_value_msg:	defm	"Enter port data (2 hex chars): ",0
;
;Simple monitor program for CPUville Z80 computer with serial interface.
monitor_first_start: ld	sp,ROM_monitor_stack
					ld	hl,monitor_message
					call	write_string
monitor_warm_start: call	write_newline		
...
help_done:	ld	a,000h
			ld	hl,buffer
			ld	(hl),a	;dj, copied the clear buffer here to see if it will show the ? again instead of dumping previous garbage
			jp	monitor_warm_start
;
...
cpm_jump:		ld	hl,0800h
			...
			call	disk_read
			jp	0800h
hello_jump: ld hl, hello_message
			call write_string
			jp monitor_warm_start
portwr_jump: ld hl, portwr_message
			call write_string
portwr_now:			ld a, ($FFFE)
			out (4), a
			ld a, ($FFFF)
			out (5), a
			;ld hl, portwr_entry_msg
			;call write_string
			;call get_char
			;call write_char
			jp monitor_warm_start
portrd_jump: ld hl, portrd_message
			call write_string
			in a, (4)
			ld ($FFFE), a
			in a, (5)
...
hello_message:		defm	"\n\rHello from DJ and Travis. We are learning!\r\n",0
portwr_message:		defm	"Writing bytes to port 4 and 5 from FFFE and FFFF.\r\n",0
portrd_message:		defm	"Reading ports 4 and 5 into FFFE and FFFF.\r\n",0
...
hello_string:		defm    "sayhi",0
portwr_string:		defm	"portwr",0
portrd_string:		defm	"portrd",0
...
parse_table:		defw	dump_string,dump_jump,load_string,load_jump
					...
					defw	hello_string,hello_jump
					defw	portwr_string, portwr_jump
					defw	portrd_string, portrd_jump
